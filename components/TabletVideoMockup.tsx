 ...
 * supporting single video, image, multiple video switcher tabs, and side-by-side dual view.
 */
export default function TabletVideoMockup({
  ...
  const isDualView = Boolean(videos && videos.length > 1 && selectedIndex === videos.length);

  return (
    <div className={`tablet-mockup-wrapper ${className}`}>
      {/* Video switcher tabs if multiple videos are provided */}
      {videos && videos.length > 1 && (
        <div className="tablet-mockup-tabs" role="tablist">
          ...
          <button
            type="button"
            role="tab"
            aria-selected={isDualView}
            className={`tablet-tab-btn ${isDualView ? "active" : ""}`}
            onClick={(e) => {
              e.preventDefault();
      ...
      {/* Main Display: Dual Grid or Single Chassis */}
      {isDualView && videos ? (
        <div className="tablet-dual-grid">
          {videos.map((v) => (
            <div key={v.label} className="tablet-dual-col">
              <div className="tablet-dual-label">
                <span>{v.label}</span>
              </div>
              <SingleTablet
                videoSrc={v.video}
